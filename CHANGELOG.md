...
- **Per-test плагин смещает `config_search_dir` РАНЬШЕ любого setup-хука и
  фикстуры.** `per_test_files` обёрнут в `pytest_runtest_protocol`-hookwrapper
  вместо обычного `pytest_runtest_setup`-хука: смещение теперь гарантированно
  происходит до самого раннего доступа к конфигу в рамках теста.

  Раньше плагин был обычным `pytest_runtest_setup`-хуком. Если конфиг собирался
  рано - фикстурой, плагином или conftest-хуком, который по LIFO-порядку
  выполнялся до плагина, - он строился от CWD (корня проекта, где стоит
  project-маркер, на котором `find_upwards`/цепочка `config.json5` немедленно
  останавливаются) и кешировался как testlocal с неправильной директорией.
  Симптом (appium): `capabilities.json5` рядом с тестом не находился →
  `ValueError: selected_device не задан`, хотя файл на месте. Симптом (selenium/
  общий): `config.json5` из директории теста игнорировался, читался корневой.
...
- **`WebmScreencastRecorder`: запись сценария больше не теряется из-за
  разноразмерных кадров.** Кодировщик собирал кадры через `np.stack`, который
  требует одинаковой формы массивов. Источник кадра (в selenium/appium-
  пакетах) переключается между способами съёмки (BiDi / CDP / классический
  скриншот), а во время навигации размеры вьюпорта/скриншота «плывут», поэтому
  в одной записи оказывались кадры разного размера. `np.stack` падал с
  "all input arrays must have the same shape", `_encode_webm` кидал
  исключение, `stop()` возвращал `None` и весь ролик терялся - на «занятых»,
  навигирующих сценариях (и тем чаще, чем выше `fps`) видео пропадало целиком.
  Теперь все кадры приводятся к размеру первого перед склейкой, так что запись
  устойчива к смене источника кадра и навигации.

## [0.2.0] - 2026-06-28

### Добавлено

- **`BaseBy` — наследуемый драйвер-независимый локатор со стратегиями.**
  Неизменяемая (frozen) `(by, value)`-пара, распаковывается в `(str, str)`
  для `find_element(*by)`. Несёт classmethod-конструкторы под общие
  W3C-стратегии (`id`, `xpath`, `name`, `class_name`, `tag_name`,
  `css_selector`, `link_text`, `partial_link_text`). Платформенные пакеты
  наследуют (`class By(BaseBy)`) и добавляют свои стратегии, не дублируя
  общие и не завися от selenium; конструкторы на `cls`, поэтому возвращают
  подкласс. Экспортирован из `tquality_core` и `tquality_core.elements`.
- **`FormattableElement[E]` — шаблонный элемент с параметризуемым
  локатором.** Хранит локатор-шаблон (placeholder'ы в синтаксисе
  `str.format`: позиционные `{}`/`{0}`, именованные `{id}`, со
  спецификаторами `{!r}`/`{:>4}`) и фабрику, собирающую конкретный элемент.
  Метод `format(*args, **kwargs)` подставляет аргументы в `value`
  локатора (стратегия `by` не меняется) и возвращает готовый к действиям
  конкретный элемент `E`. Параметризуется типом локатора `L` (любой подкласс
  `BaseBy`, по умолчанию `BaseBy`), поэтому принимает платформенный `By`
  напрямую и сохраняет его конкретный тип в результате `format()`.
  Намеренно не наследует `BaseElement`: до `format()` искать нечего.
  Экспортирован из `tquality_core` и `tquality_core.elements`.

### Изменено

- **`BaseElement` — тонкий интерфейс на композиции, локатор `BaseBy`.**
  Ожидания вынесены в композицию: вместо набора `wait_until_*`/`wait_for_*`
  методов на элементе интерфейс требует свойство `wait` (платформенный
  `ElementWaiter`, в ядре типизирован как `Any`) - `element.wait.until_*()`.
  Контракт: `name`, `by` (тип `BaseBy`), `text`, `is_displayed`/`is_present`/
  `is_enabled`, `wait`, `get_attribute`, `click`. Прежний `Locator`
  (NamedTuple) удалён - его роль занял `BaseBy`.
- **`ElementState.EXISTS_IN_ANY_STATE` ждёт присутствия элемента.** Раньше
  состояние трактовалось как «не проверять ничего - сразу `_find()` и
  действие». Теперь перед действием элемент ожидается через
  `wait.until_present()`: существование в дереве (в любом состоянии -
  видимость/кликабельность по-прежнему не проверяются). Платформенные
  `BaseElement._await_state(...)` должны мапить это состояние на
  `until_present`.
...
- **HTTP-клиент** (`tquality_core.http_client`) — опциональный компонент,
  устанавливается с extra `http_client` (тянет `requests` и `urllib3`). 
  Импорт подпакета без extra даёт `ModuleNotFoundError`.
  Публичный API — на уровне пакета, модули-реализация скрыты под `_`-префиксом.
  - `BaseClient` — обёртка над `requests.Session` с типизированными методами
    `_get`/`_post`/`_put`/`_patch`/`_delete` (и базовым `_request`), единым
    `base_url`, общими заголовками, cookies, таймаутом и ретраями (`Retry`) на
    уровне клиента. Логирует строку запроса и ответа (метод, URL, статус, время):
    через переданный `Logger`, иначе — через активный логгер шагов (новый
    `current_logger()`, если резолвер настроен). Заголовки и тело не
    пишутся, чтобы не утекали секреты.
  - `ApiResponse[T]` — наследник `requests.Response` с ленивым потокобезопасным
    свойством `.data`, валидирующим тело в pydantic-модель (или union) через
    `TypeAdapter`. Тип `.data` — ровно `T`: `None` появляется, только если он явно
    в модели либо модель не задана.
  - **XML-ответы** — extra `xml` (слой поверх `http_client`, тянет `pydantic-xml`):
    если модель ответа — наследник `BaseXmlModel`, тело разбирается из байтов через
    `from_xml` вместо JSON. Без extra ветка XML мягко отключается.
  - `Headers` — pydantic-модель частых заголовков со snake_case-полями,
    сериализуемыми в канонический `Header-Case`; значения по умолчанию, опциональные
    заголовки-подсказки (в т.ч. `X-API-Key`, `X-IBM-Client-Id`), `extra="allow"`.
  - `ContentType` — `StrEnum` частых MIME-типов для `Content-Type`/`Accept`.
  - `TimeoutHTTPAdapter` (таймаут в секундах по умолчанию) и `RequestArgsDict`
    (типизированные `**kwargs` запроса).
