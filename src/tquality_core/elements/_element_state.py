...
# работает - ждём только присутствия элемента в дереве.
...
class ElementState(Enum):
    ...
    """Ждать `wait.until_present()` (элемент существует в дереве, состояние
    любое - видимость/кликабельность не проверяются). Полезно для элементов,
    которые не соответствуют стандартам видимости/кликабельности
